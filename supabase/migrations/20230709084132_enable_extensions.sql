...
declare
  ...
  ret varchar;
begin
  r := extensions.gen_random_bytes(10);
...
create function tz_to_iso(tz timestamp with time zone)
...
create function to_b64u(val bytea)
returns text
language sql
immutable
as $$
...
create function from_b64u(val text)
returns bytea
language sql
immutable
as
...
begin
  ...
  ret := decode(lpad(to_hex(ct), 12, '0'), 'hex') || extensions.gen_random_bytes(26);
  return ret;
end;
$$
;
